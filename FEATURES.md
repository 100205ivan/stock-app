...
**Q: 需要什麼環境才能運行此應用？**  
A: 需要 Node.js 18+、npm 或 yarn，以及 Expo Go App（用於在實體裝置上測試）。

**Q: 如何在實體手機上測試？**  
A: 安裝 Expo Go App，執行 `npm start`，然後掃描終端機顯示的 QR code。

**Q: 可以在網頁瀏覽器上運行嗎？**  
A: 可以，執行 `npm run web` 即可在瀏覽器中開啟。

### 功能使用

**Q: 如何新增自選股？**  
A: 進入股票詳情頁，點擊右上角的星號圖示即可加入或移除自選股。

**Q: 投資組合的損益如何計算？**  
A: 系統會根據您記錄的買入/賣出交易，自動計算平均成本，並與當前股價比較得出損益。

**Q: 回測功能使用的是真實數據嗎？**  
A: 是的，「買進持有」策略使用 Yahoo Finance 的真實歷史收盤價進行回測。其他策略目前為示意功能。

**Q: 價格提醒會推播通知嗎？**  
A: 目前價格提醒功能僅儲存設定，尚未實現背景推播機制，需要手動進入 App 檢查。

### 資料與隱私

**Q: 我的交易記錄會上傳到雲端嗎？**  
A: 不會。所有資料（自選股、交易記錄、提醒）都儲存在您的裝置本地，不會上傳。

**Q: 解除安裝 App 後資料會保留嗎？**  
A: 不會。解除安裝會清除所有本地資料。未來版本將加入雲端備份功能。

**Q: 需要註冊帳號嗎？**  
A: 目前不需要。所有功能都可以在本地使用，無需登入。

### 技術問題

**Q: 為什麼有些股票沒有 Logo？**  
A: 部分股票的 Logo 可能無法從 API 獲取，系統會自動顯示彩色縮寫作為替代。

**Q: 股價更新頻率如何？**  
A: 股價資料來自第三方 API，更新頻率取決於 API 提供商。一般為延遲 15-20 分鐘的報價。

**Q: 技術指標（RSI、MACD）準確嗎？**  
A: 目前技術指標為示意數值，非真實計算結果。真實技術指標計算功能正在開發中。
...
- **啟動時間**：< 2 秒（首次啟動）
- **頁面切換**：< 300ms
- **API 回應時間**：平均 500-1000ms（取決於網路）
- **記憶體使用**：約 80-120MB

### 優化措施
- 使用 `FlatList` 優化長列表渲染
- 圖片 lazy loading 減少初始載入時間
- API 請求快取機制
- 錯誤處理與自動重試
...
- 🔄 新增下拉刷新功能
  ...
  - 視覺化載入動畫
- 🎨 新增公司 Logo 顯示
  - 熱門、台股、美股、自選股全部顯示 Logo
  - 無 Logo 股票自動顯示彩色縮寫
  - 元大、中信金、台塑、南亞、國泰金等30+股特殊色彩配置
- 🐛 修復股價百分比計算錯誤
- 🐛 修復搜尋歷史導航至股票詳情頁問題
- 🔧 改進股票詳情頁即時價格獲取
